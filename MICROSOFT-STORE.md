...
CI additionally copies the unsigned Store artifact, signs that disposable copy with an ephemeral test certificate, installs it, verifies the package family, launches the executable with `--version`, uninstalls it and deletes the test certificate. The ephemeral test-signed copy is never uploaded or published.
...
- CI successfully built, unpacked, locally signed, installed, launched and uninstalled a disposable copy.
